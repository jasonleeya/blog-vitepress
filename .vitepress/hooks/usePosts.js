...
const postList = ref(JSON.parse(JSON.stringify(_postList)));
const hotList = ref([]);
export function usePosts() {
    const postsTotal = _postList.length;
    const getAllPosts = () => {
        postList.value = JSON.parse(JSON.stringify(_postList));
    };
    const groupPostListByTag = (tag) => {
        const list = JSON.parse(JSON.stringify(_postList));
        if (!tag) {
            postList.value = list;
        }
        else {
    ...
    const categoryInfo = useData().theme.value.categoryInfo;
    ...
    const groupPostListByCategory = (category) => {
        const list = JSON.parse(JSON.stringify(_postList));
        if (!category) {
            postList.value = list;
        }
    ...
    const getHotPostList = async () => {
        return axios.post('/getHotPosts').then(res => {
