...
- **`clientTop`**：元素**上边框的`宽度`**（即 `border-top-width`）
...
`for...await...of` 是 JavaScript 中用于**遍历异步可迭代对象**的语法，专门处理需要按顺序等待的异步操作（例如从分页接口逐页获取数据或读取大文件流）。它的核心作用是**在循环中等待每个异步任务完成后再处理下一个值**，确保顺序性和可控性。

**核心作用与示例**

1. **遍历异步数据源**：  
   适用于处理异步生成器（Async Generator）、返回 Promise 的集合或 Node.js 流（Stream）。  
   ```javascript
   // 示例：遍历异步生成器（逐页获取数据）
   ...
   // 使用 for...await...of 逐页处理
   (async () => {
     for await (const pageData of fetchPages()) {
       console.log("处理数据:", pageData);
     }
   })();
   ```

2. **处理流式数据**：  
   如逐行读取文件或处理 WebSocket 消息：  
   ```javascript
   // 示例：Node.js 中读取文件流
   const fs = require('fs');
...
| ------------ | ------------------------ | ---------------------------------- |
| **适用对象** | 同步可迭代对象（如数组） | 异步可迭代对象（如异步生成器、流） |
| **等待机制** | 同步执行，不等待 Promise | 自动等待每个 Promise 完成          |
| **使用场景** | 同步数据处理             | 异步任务序列（如分页、流处理）     |

**注意事项**

1. **必须在 Async 函数内使用**：  
   外层需包裹 `async` 函数，否则会报语法错误。  
   ```javascript
   // ❌ 错误写法（不在 async 函数中）
...
2. **错误处理**：  
   如果异步迭代中某个 Promise 被拒绝（Reject），循环会抛出异常，需用 `try...catch` 捕获：  
   ```javascript
   try {
...
3. **性能考量**：  
   循环会**串行执行**异步操作，若任务无依赖，可用 `Promise.all` 并行优化。

**可能追问的问题**

1. **Q: 如何手动实现一个异步可迭代对象？**  
   
   **A**: 定义对象的 `Symbol.asyncIterator` 方法，返回一个生成器，其 `next()` 方法返回 Promise：  
...
2. **Q: 如果在循环中某个 Promise 被拒绝会怎样？**  
   
   **A**: 循环立即终止并抛出错误，后续迭代不会执行。需用 `try...catch` 包裹整个循环或每个 `await`。
   
3. **Q: 和 `Promise.all` 遍历有什么区别？**  
   
   **A**:  
   
   - `for...await...of` 是**串行执行**，适合有顺序依赖的任务（如分页）。  
   - `Promise.all` 是**并行执行**，适合独立任务且需等所有完成（如批量请求）。
...
1. **Q: `obj?.prop` 和 `obj && obj.prop` 有什么区别？**

   **A**: 功能类似，但可选链更简洁且直接支持深层嵌套（如 `a?.b?.c`）。

2. **Q: 为什么不能直接用 `||` 代替 `??`？**

   **A**: `||` 会覆盖所有假值（如 `0`、`false`），而 `??` 保留这些有效值。

3. **Q: `?.` 可以用于赋值吗？如 `obj?.prop = 1`？**

   **A**: 不可以，会报语法错误。可选链仅用于读取值。

4. **Q**:**可选链能否用于数组或函数？**

   **A**:可以。例如：

   - 数组安全访问：`arr?.[index]`（若 `arr` 为 `null/undefined`，返回 `undefined`）；
   - 函数安全调用：`fn?.()`（若 `fn` 不存在，不报错）。
...
1. **Q: `Symbol` 能实现真正的私有属性吗？**

   **A**: 不能完全私有，但通过闭包 + `Symbol` 可达到类似效果。ES2022+ 的 `#` 语法才是真正的私有字段。

2. **Q: 如何获取对象的所有 `Symbol` 属性？**

   **A**: 用 `Object.getOwnPropertySymbols(obj)`。

3. **Q: `Symbol.for(key)` 和 `Symbol(key)` 的区别？**

   **A**:

   - `Symbol(key)` 每次创建唯一值。
   - `Symbol.for(key)` 先检查全局注册表，存在则复用。
...
**追问 1：如何改变箭头函数的 this 指向？**

**回答**：
无法改变。箭头函数的 `this` 继承自外层，不可通过 `call`、`apply`、`bind` 方法修改。

**追问 2：箭头函数和普通函数的性能差异？**

**回答**：
箭头函数的解析速度略快（因语法简单），但执行效率无显著差异。实际开发中，性能通常不是选择箭头函数的主要因素。

**追问 3：箭头函数能否使用 `super` 关键字？**

**回答**：
箭头函数本身无 `super`，但可继承自外层函数或类。
...
- 非简单请求（如 PUT/DELETE、自定义请求头）会先发送 OPTIONS 请求验证权限，需服务端正确响应
...
1. **Q: 为什么 `0.1` 在二进制中是无限循环的？**

   **A**: 十进制小数转换为二进制时，若无法整除 2，则会无限循环。例如：
...
2. **Q: 如何判断一个数是否超出安全整数范围？**

   **A**: 用 `Number.isSafeInteger(num)`。

3. **Q: `BigInt` 和 `Number` 能直接运算吗？**

   **A**: 不能，需显式转换：`Number(1n) + 2` 或 `1n + BigInt(2)`。
...
1. **模块缓存**
   两者都有缓存机制，同一模块仅执行一次（CJS 基于绝对路径，ESM 基于模块标识符）。
2. **依赖管理**
   都解决了代码复用和依赖关系问题，但实现方式不同
...
**追问 1：什么是 Tree Shaking？为什么 ESM 支持而 CJS 不支持？**

**回答**：
Tree Shaking 是指打包工具（如 Rollup、Webpack）通过静态分析，剔除未使用的 ESM 导出代码。CJS 因动态加载特性（如条件 require）无法在编译时确定依赖关系，故不支持。
...
**追问 1：为什么需要 Reflect？直接操作 target 不可以吗？**

**回答**：
使用 Reflect 而非直接操作 `target` 的原因：

- **保持行为一致性**：`Reflect` 方法与 Proxy 陷阱的默认行为完全一致，避免手动实现可能引入的边缘问题。
- **处理特殊对象**：对于某些内置对象（如 `Map`、`Set`），直接操作可能破坏其内部状态，而 `Reflect` 能正确处理。

**追问 2：Proxy 能拦截哪些操作？**

**回答**：
Proxy 可拦截 13 种操作，常见的包括：
...
**追问 3：Proxy 和 Object.defineProperty () 的区别？**

**回答**：

| **特性**     | **Proxy**                      | **Object.defineProperty()**         |
| ------------ | ------------------------------ | ----------------------------------- |
| **拦截粒度** | 可拦截对象的所有操作           | 仅能拦截属性的读取 / 设置           |
| **深层监听** | 默认不递归，需手动处理嵌套对象 | 默认不递归，需手动处理嵌套对象      |
| **新增属性** | 可拦截                         | 无法拦截新增属性                    |
| **数组操作** | 可拦截 `push`、`pop` 等方法    | 部分数组方法（如 `length`）无法拦截 |

## 你对 Set、Map 两种数据结构怎么理解?

**一、核心概念与区别**

**Set** 和 **Map** 是 ES6 引入的两种数据结构，用于存储数据集合，但核心区别在于：

- **Set**：无序且唯一的值的集合（类似数学中的集合），不存储键值对，仅存储值本身。
- **Map**：键值对的集合，键和值可以是任意类型，且键具有唯一性。

**二、主要特性对比**

| **特性**       | **Set**                        | **Map**                               |
| -------------- | ------------------------------ | ------------------------------------- |
| **存储内容**   | 唯一值的集合（如 `[1, 2, 3]`） | 键值对的集合（如 `{k1: v1, k2: v2}`） |
| **键的类型**   | 不涉及键，仅存储值             | 键可以是任意类型（包括对象、函数）    |
| **键的唯一性** | 值不可重复（重复值会被忽略）   | 键不可重复（重复键会覆盖旧值）        |
| **迭代顺序**   | 按插入顺序迭代                 | 按插入顺序迭代                        |
| **常见用途**   | 去重、成员检测（如 `has`）     | 缓存、关联数据（如 DOM 节点映射）     |

**三、注意点**

1. **值的唯一性判断**

   **Set** 和 **Map** 的键都使用 **SameValueZero** 算法判断相等（类似 `===`，但 `NaN` 被视为相等）：
...
2. **性能优势**
   ...
   - **Set** 的 `has` 操作时间复杂度为 **O(1)**（基于哈希表），远快于数组的 `includes`（O(n)）。

   - **Map** 的键可以是对象，而普通对象的键只能是字符串或 Symbol：
...
3. **迭代方式**
   ...
   - **Set** 和 **Map** 都支持 `for...of` 和 `forEach`，但 Map 的迭代器返回 `[key, value]` 数组：
...
- **Set 的典型应用场景**
  ...
  1. **数据去重**:Set 自动过滤重复值，无需手动比较（如 `[...new Set([1, 2, 2])]`）,事件复杂度为 **O（1）**，比数组的 `includes`（O (n)）快得多。

  2. **成员检测（频繁查找）**：判断元素是否存在（如权限校验、黑名单过滤）

  3. **集合运算（交集、并集、差集）**：原生支持迭代，结合数组方法（如 filter）可简洁实现集合运算。
...
- **Map 的典型应用场景**
  ...
  1. **键值对存储（键为对象或复杂类型）**:键为 DOM 节点、函数等关联数据（如缓存、状态管理），普通对象的键只能是字符串或 Symbol，而 Map 的键可以是任意类型。

     ```javascript
     const nodeMap = new Map();
     const btn = document.createElement('button');
     nodeMap.set(btn, { count: 0 }); // 合法
     ```

  2. **需要保持插入顺序**:实现 LRU 缓存、事件队列等依赖顺序的数据结构，Map 按插入顺序迭代，而普通对象的键顺序不保证（ES6 后字符串键按插入顺序，但非强制）

  3. **频繁增删键值对**：实时更新数据（如购物车、实时统计），Map 的 `set`、`delete` 操作性能优于普通对象（对象的 `delete` 可能导致内存碎片）

**潜在追问**

**追问 1：Set 和数组（Array）的区别？**

**回答**：

- **Set**：元素唯一、无索引、插入 / 删除效率高（O (1)），适合去重和成员检测。
- **Array**：元素可重复、有索引、支持排序和各种操作（如 `map`、`filter`），适合需要索引或顺序的场景。

**追问 2：Map 和普通对象（Object）的区别？**

**回答**：

| **特性**     | **Map**                  | **普通对象**                                     |
| ------------ | ------------------------ | ------------------------------------------------ |
| **键的类型** | 任意类型（对象、函数等） | 只能是字符串或 Symbol                            |
| **键的顺序** | 按插入顺序迭代           | 不保证顺序（ES6 后字符串键按插入顺序，但非强制） |
| **默认属性** | 仅内置方法（如 `size`）  | 原型链上可能存在默认属性                         |
| **性能**     | 频繁增删键值对时更快     | 适合简单数据存储                                 |

追问 3：如何将 Set/Map 转换为数组？

**回答**：

- 使用扩展运算符 `...` 或 `Array.from`：
...
- Map 转换为数组时会生成 `[key, value]` 对的数组：

  ```javascript
  const map = new Map([['a', 1]]);
  const arr = Array.from(map); // [['a', 1]]
  ```
...
| **特性**     | **Map**                                 | **Object**                                       |
| ------------ | --------------------------------------- | ------------------------------------------------ |
| **键的类型** | 任意类型（对象、函数、基本类型）        | 仅字符串或 Symbol                                |
| **键的顺序** | 按插入顺序迭代                          | 不保证顺序（ES6 后字符串键按插入顺序，但非强制） |
| **默认属性** | 仅内置方法（如 `size`）                 | 原型链上可能存在默认属性（如 `toString`）        |
| **性能**     | 频繁增删键值对时更快                    | 适合简单数据存储                                 |
| **序列化**   | 不支持直接序列化（如 `JSON.stringify`） | 支持序列化（需处理循环引用）                     |

**潜在追问**

追问 3：如何选择使用 Map 还是 Object？
...
- **requestAnimationFrame**：
  - 在浏览器**下一帧渲染前**执行回调（通常为 16.6ms 间隔，取决于屏幕刷新率）。
  - 优化动画渲染，避免卡顿（与浏览器刷新周期同步）。
- **requestIdleCallback**：
  - 在浏览器**空闲时段**执行回调（如帧渲染完成后仍有剩余时间）。
  - 执行低优先级任务（如数据预处理、UI 懒加载），避免阻塞主线程。

**2. 触发时机**

- **rAF**：
  - **与屏幕刷新率同步**（60hz间隔为16.6ms），回调函数在重绘前的布局计算阶段执行。
- **rIC**：
...
**追问 1：为什么 RAF 比 setInterval 更适合动画？**

**回答**：

- RAF 会在浏览器的下一次重绘之前执行回调函数，而`setInterval`会按照指定的时间间隔重复执行回调函数。
- RAF 会自动考虑浏览器的重绘，避免不必要的重绘，提供更流畅的动画效果。而`setInterval`则不会考虑浏览器的重绘，可能会导致不必要的重绘，影响性能。
- RAF 会在浏览器的后台标签页中暂停，避免不必要的计算资源占用。而`setInterval`则会一直执行，可能会导致浏览器卡顿。

**追问 2：如何在 RIC 中处理耗时任务？**

**回答**：

- 将任务拆分为小单元，通过 `timeRemaining()` 判断是否继续执行：

  ```javascript
  function processChunk(tasks, deadline) {
    while (deadline.timeRemaining() > 0 && tasks.length > 0) {
      const task = tasks.pop();
      task();
    }
    if (tasks.length > 0) {
      requestIdleCallback((nextDeadline) => processChunk(tasks, nextDeadline));
    }
  }
  ```

**追问 3：RAF 和 RIC 可以嵌套使用吗？**

**回答**：

- 可以，但需谨慎控制。例如，在 RAF 中执行动画更新后，可通过 RIC 处理后续数据同步：
...
| ------------ | -------------------------------- | -------------------------------------------- |
| **成员类型** | 仅包含元素节点（如 `<div>`）     | 包含所有节点（元素、文本、注释等）           |
| **动态性**   | 实时反映 DOM 变化（动态）        | 多数情况下静态（如 `querySelectorAll`）      |
| **方法**     | 仅支持 `item()` 和 `namedItem()` | 支持 `forEach`、`entries` 等迭代方法         |
| **获取方式** | 通过 `getElementsBy*` 方法返回   | 通过 `querySelectorAll`、`childNodes` 等返回 |



## 你知道哪些设计模式？

在前端开发中，常用的设计模式大概可以分为创建型、结构型和行为型三类。我结合实际场景来聊聊几个典型的吧。

**创建型模式**里，单例模式比较常见，比如全局状态管理工具（像早期的 Redux Store），确保一个应用中只存在一个实例，避免资源重复创建。工厂模式则适合批量生成相似对象，比如用一个工厂函数根据不同参数创建不同类型的按钮组件，减少重复代码。

**结构型模式**中，装饰器模式在 React 里很常用，比如用高阶组件（HOC）为组件添加日志、权限等功能，不修改原组件代码就能扩展功能。适配器模式适合处理数据格式不兼容的情况，比如后端返回的数据结构和前端组件需要的格式不一致时，通过适配器转换格式，让双方顺利对接。

**行为型模式**里，观察者模式（发布 - 订阅模式）是核心，比如 Vue 的响应式系统，当数据变化时自动通知订阅者更新视图。策略模式可以优化条件判断逻辑，比如根据不同的用户角色（普通用户、管理员），调用不同的权限验证策略函数，将策略逻辑从主流程中解耦。
...
**2. 观察者模式（Observer）/ 发布-订阅（Pub-Sub）**
...
- **场景**：Vue的响应式原理、数据校验、缓存代理（如 HTTP 请求缓存）、ES6 的 `Proxy` 对象。
...
- **示例**：
  ...
  **可能的追问**：
  ...
  1. **追问：Vue 3 为什么用组合式 API 替代部分设计模式？**
     **回答**：组合式 API（setup、ref）通过逻辑组合替代高阶组件等模式，更直观地复用状态逻辑，避免嵌套地狱，同时解决了 React 类组件中 this 指向的问题。
  2. **追问：前端中如何实现一个发布 - 订阅模式？**
     **回答**：核心是维护一个事件池对象，用 on 方法订阅事件，emit 方法触发事件，off 方法取消订阅。比如 Vue 的 *o**n*/emit 或现代的 EventEmitter 类。
  3. **追问：什么时候该用策略模式？举个例子。**
     **回答**：当业务逻辑有多个条件分支（如不同支付方式的计算逻辑），且后续可能新增策略时，将每个策略封装为独立函数，通过上下文对象动态调用，方便扩展和维护。
...
```javascript
// 发送消息
window.opener.postMessage('你好，父窗口!', 'https://example.com');
...
1. **问：这些方式中哪些是实时的，哪些有延迟？**
   答：WebSocket 和 Broadcast Channel 是实时的，localStorage 有延迟。
2. **问：如果需要支持不同源的标签页通信，该怎么做？**
   答：可以使用 postMessage，它支持跨域通信。
3. **问：Broadcast Channel 在不同浏览器中的兼容性如何？**
   答：主流浏览器都支持，但 IE 不支持，需要做兼容性处理。
4. **问：使用 localStorage 通信时，如果多个标签页同时修改数据，会有冲突吗？**
   答：会有冲突，需要实现锁机制或版本控制来解决。
5. **问：Service Worker 在跨标签页通信中有什么优势和劣势？**
   答：优势是可以离线工作，劣势是生命周期复杂，注册和激活过程可能失败。

## 你对 token 进行身份验证了解多少？

Token 身份验证是一种无状态的身份验证机制，主要流程是用户登录后，服务器生成一个 Token 返回给客户端，客户端在后续请求中携带这个 Token，服务器通过验证 Token 来确认用户身份。以下是详细介绍：

**核心概念**

1. **Token 类型**
   - **JWT（JSON Web Token）**：由 Header、Payload、Signature 三部分组成，常用于前后端分离的应用。
   - **OAuth 2.0 Token**：主要用于第三方登录授权，例如微信登录、Google 登录。
   - **Session Token**：服务器生成的随机字符串，与服务器端的 Session 对应。
2. **工作流程**
   - 用户登录时提交用户名和密码。
   - 服务器验证通过后生成 Token 并返回。
   - 客户端将 Token 存储在 LocalStorage、Cookie 或 Memory 中。
   - 后续请求在 Header 中携带 Token（如`Authorization: Bearer {token}`）。
   - 服务器验证 Token 的有效性，返回相应资源。
3. **安全性考虑**
   - **防止 CSRF 攻击**：JWT 不存储在 Cookie 中，使用 LocalStorage 存储；Session Token 需要配合 CSRF Token。
   - **防止 XSS 攻击**：避免 Token 被脚本获取，可设置 HttpOnly 属性（仅对 Cookie 有效）。
   - **Token 过期策略**：设置合理的过期时间，使用 Refresh Token 延长会话。

**可能的追问及回答**

1. **追问**：JWT 和 Session Token 的区别是什么？
   - **回答**：JWT 是无状态的，数据存储在客户端；Session Token 是有状态的，数据存储在服务器端。JWT 更适合微服务架构和跨域场景。
2. **追问**：如何防止 Token 被篡改？
   - **回答**：JWT 通过 Signature 确保数据完整性，服务器端验证签名时会重新计算哈希值并比对。
3. **追问**：Refresh Token 的工作原理是什么？
   - **回答**：Access Token 过期后，客户端使用 Refresh Token 向服务器换取新的 Access Token，避免用户重新登录。
4. **追问**：Token 存储在 LocalStorage 和 Cookie 各有什么优缺点？
   - **回答**：LocalStorage 可被 JavaScript 访问，易受 XSS 攻击，但支持跨域；Cookie 可设置 HttpOnly 防止 XSS，但有 CSRF 风险，且不支持跨域。
5. **追问**：在分布式系统中，如何保证 Token 验证的一致性？
   - **回答**：使用共享存储（如 Redis）保存 Token 状态，或采用无状态的 JWT，所有服务使用相同的密钥验证。

## 如何安全存储 JWT？

- JWT 的安全存储是防止身份盗用的关键环节，需要根据应用场景权衡存储位置、生命周期和防护措施。以下是我的核心观点：

  **一、存储位置选择**

  1. **LocalStorage / SessionStorage**
     - **优点**：可被 JavaScript 访问，适合 SPA 前端逻辑
     - **缺点**：**易受 XSS 攻击**（脚本可直接读取）
     - **安全建议：**
       - 仅存储短期有效的**Access Token**（5-15 分钟）
       - 配合**HttpOnly 的 CSRF Token**防御 XSS
       - 使用`Secure`属性确保 HTTPS 传输
  2. **HttpOnly Cookie**
     - **优点**：**防 XSS**（JavaScript 无法访问）、自动随请求发送
     - **缺点**：**易受 CSRF 攻击**（同源策略限制）
     - **安全建议**：
       - 存储长期的**Refresh Token**（7-30 天）
       - 设置`SameSite=Strict/Lax`限制跨站请求
       - 结合**CSRF Token**双重防护
  3. **Memory Storage**
     - **优点**：不持久化，页面刷新后丢失，适合临时存储
     - **缺点**：页面刷新后需重新认证
     - **场景**：敏感操作（如支付）的临时 Token

  **二、Token 生命周期管理**

  1. **双 Token 机制**

     - **Access Token**：短期有效，存于 LocalStorage/Memory

     - **Refresh Token**：长期有效，存于 HttpOnly Cookie

     - **流程**

       ```plaintext
       Access Token过期 → 前端用Refresh Token请求新Token → 后端验证并生成新Token对
       ```

  2. **Token 刷新策略**

     - **自动刷新**：静默刷新（如 Token 剩余有效期 < 20% 时）
     - **手动刷新**：用户交互后刷新（如点击 "重新登录"）
  ...
  **三、防护措施**
  ...
  1. **防 XSS 攻击**
     ...
     - 输入输出转义，避免 HTML 注入
     - Content Security Policy (CSP) 限制脚本源
     - 限制 Token 权限粒度（如只读 Token、操作 Token 分离）

  2. **防 CSRF 攻击**

     - HttpOnly Cookie + Custom Header 验证

     - 双重提交 Cookie 模式（Cookie 与请求参数包含相同随机值）

     - SameSite 属性配置：

       ```plaintext
       SameSite=Strict   # 严格禁止跨站请求
       SameSite=Lax      # 允许GET请求跨站
       ```

  3. **Token 撤销机制**

     - **黑名单**：将已注销的 Token 加入 Redis 黑名单
     - **版本控制**：用户密码修改时更新 Token 版本号

  

**可能的追问及回答**

1. **追问**：如何在 SPA 中同时防御 XSS 和 CSRF？

   **回答：**

   - Access Token 存 LocalStorage + HttpOnly 的 CSRF Token
   - Refresh Token 存 HttpOnly Cookie + SameSite=Lax
   - 所有非 GET 请求携带自定义 Header（如`X-CSRF-Token`）

2. **追问**：Refresh Token 被盗用后如何补救？

   **回答：**

   - 后端检测异常登录，强制失效相关 Refresh Token
   - 多因素认证（如设备绑定、短信验证码）
   - 缩短 Refresh Token 有效期

3. **追问：**为什么 HttpOnly Cookie 能防 XSS？

   **回答：**浏览器禁止 JavaScript 读取 `HttpOnly` 的 Cookie，即使 XSS 注入也无法窃取令牌。

4. **追问：**前端如何无感刷新 Token？

   **回答：**在拦截器中捕获 401 错误，用 Refresh Token 静默获取新 Token：
...
1. **Q: `reduce` 的第二个参数（初始值）可以省略吗？**
   **A**: 可以，但空数组会报错（无初始值时从数组第一项开始迭代）。

2. **Q: 如何用 `reduce` 实现 `map` 或 `filter`？**
   **A**:
...
3. **Q: `reduce` 和 `reduceRight` 的区别？**
   **A**: `reduceRight` 从右向左迭代（如处理函数组合时顺序相反）。

## 函数修改形参，能否影响实参？

在 JavaScript 中，函数修改形参是否会影响实参，取决于参数的**数据类型**和**操作方式**。

对于**基本类型**（如`number`、`string`、`boolean`、`null`、`undefined`、`symbol`），修改形参**不会影响**实参。
**原因**：基本类型传递的是值的副本，形参和实参指向不同的内存空间。
对于**引用类型**（如`object`、`array`、`function`），修改形参**可能影响**实参，需分情况讨论：

- **修改对象 / 数组的属性 / 元素**会影响实参，因为形参和实参指向同一个对象 / 数组。

- **重新赋值整个对象 / 数组**不会影响实参，因为形参被指向了新的内存空间，与实参断开联系：
  ...
  let arr = [1, 2, 3];
  reassignArray(arr);
  console.log(arr); // 输出: [1, 2, 3]（未受影响）
  ```

  **可能的追问及回答**

  1. **追问**：JavaScript 中所有参数传递都是值传递吗？

     - **回答**：是的。基本类型传递值的副本，引用类型传递引用的副本（指向同一对象），但本质上都是值传递。

  2. **追问**：如何在函数内部安全地修改对象而不影响实参？
...
迭代器（Iterator）是 JavaScript 中处理集合数据的一种统一机制，它提供了一种标准方式来顺序访问一个数据结构中的各个元素，而不需要暴露该数据结构的内部实现细节。以下是我对迭代器的核心理解：
...
生成器（Generator）是 JavaScript 中一种特殊的函数，它允许你定义一个迭代算法，并通过暂停和恢复执行的机制来控制数据流。以下是我对生成器的核心理解：
...
1. **追问**：生成器和迭代器的关系是什么？
   ...
   - **回答**：生成器是迭代器的语法糖。生成器函数返回的对象自动实现了迭代器协议（`next()`、`return()`、`throw()`），简化了手动创建迭代器的过程。

2. **追问**：如何用生成器实现一个斐波那契数列？
...
3. **追问**：生成器在现代 JS 中的替代方案是什么？

   - **回答**：
     异步场景中，`async/await` 已基本替代生成器处理异步流，其语法更简洁且错误处理更直观。生成器目前主要用于惰性计算和自定义迭代逻辑。

4. **追问**：`for...of` 循环如何处理生成器返回的值？

   - **回答**：
     `for...of` 会自动忽略 `done: true` 时的 `value`，只处理 `done: false` 的值。因此生成器中通常用 `yield` 而非 `return` 返回数据。

## 谈谈你对V8引擎的理解

V8 引擎是 Google 开发的高性能开源 JavaScript 引擎，广泛应用于 Chrome 浏览器和 Node.js。其核心目标是高效执行 JavaScript 代码，通过 **即时编译（JIT）**、**内存管理优化** 和 **运行时特性支持** 实现这一目标。以下是关键理解：

**1. 核心工作机制**

V8 的架构可以概括为「解析→执行→优化」的流水线，核心组件包括

**(1) 解析与编译流水线**  

- **解析器（Parser）**：

  第一步是把 JS 代码「拆解」成计算机能理解的结构。词法分析器（Lexer）先把代码拆成一个个「词元」（比如 `function`、`let`），然后语法分析器（Parser）把这些词元组合成**抽象语法树（AST）**，就像把零散的单词组成有逻辑的句子。  

- **解释器（Ignition）**：

  有了 AST 后，解释器 Ignition 会把它翻译成**字节码**（类似机器码的中间语言）。字节码更轻量，能快速启动执行，适合处理一次性或低频代码。  

- **优化编译器（TurboFan）**：

  对于高频执行的「热点代码」（比如循环、被多次调用的函数），V8 会用 TurboFan 编译器把字节码编译成**优化后的机器码**（类似原生程序），大幅提升执行速度。如果后续代码的类型变化（比如函数参数类型改变），优化会被「撤销」，回退到字节码执行（称为「去优化」）。

**2. 内存管理：垃圾回收（GC）**  

V8 采用 **分代回收策略**，针对不同生命周期的对象优化：  
...
- **新生代**（Young Generation）：**Scavenge 算法**
  ...
  **(1) 内存结构**
  ...
  - **From 空间**：当前使用中的内存区域。
  - **To 空间**：空闲内存区域（与 From 大小相同）。

  **(2) 回收过程**

  1. **标记存活对象**：遍历 From 空间，标记仍被引用的对象。
  2. **复制存活对象**：将存活对象按顺序复制到 To 空间。
  3. **清空 From 空间**：释放 From 空间所有内存（无需碎片整理）。
  4. **角色交换**：From 和 To 空间交换，完成回收。

  **(3) 对象晋升（Promotion）**

  满足以下条件时，对象从新生代晋升到老生代：

  - **存活次数超过阈值**（通常 2 次）。
  - **To 空间使用率超过 25%**：防止复制后空间不足。

- **老生代**（Old Generation）：**标记-清除（Mark-Sweep）与标记-整理（Mark-Compact）**

  **(1) 标记-清除**

  - **标记阶段**：从根对象（全局变量、活动函数栈）出发，递归标记所有可达对象。
  - **清除阶段**：遍历内存，释放未标记对象占用的空间。
  - **问题**：产生内存碎片。

  **(2) 标记-整理**

  - **标记阶段**：同标记-清除。
  - **整理阶段**：将存活对象向内存一端移动，清除边界外的内存（解决碎片）。

  **(3) 优化策略**

  - **增量标记（Incremental Marking）**：将标记过程拆分为多个小任务，穿插在代码执行中，减少单次停顿时间。
  - **惰性清理（Lazy Sweeping）**：延迟清理未标记内存，避免集中处理导致卡顿。
  - **并发标记/清理**：利用多线程在后台执行标记/清理，不阻塞主线程。
...
- **作用**：优化对象属性访问速度。  
- **原理**：相同结构的对象共享隐藏类，属性偏移固定。  
- **陷阱**：动态添加/删除属性会触发隐藏类变更，降低性能。  

**示例**：  

```javascript
// 隐藏类优化
const obj1 = { a: 1 };       // 隐藏类 C0
obj1.b = 2;                 // 切换为隐藏类 C1

// 隐藏类失效（影响性能）
const obj2 = { a: 1 };
obj2.b = 2;                 // 切换为隐藏类 C1（与 obj1 共享）
delete obj2.a;              // 触发新隐藏类 C2，优化失效
```
...
- **原理**：缓存对象的隐藏类和属性偏移，减少动态查找。  

**(3) 懒解析（Lazy Parsing）：**只解析当前需要执行的代码，没用到的函数暂时不解析，节省启动时间。

**4. 关键设计取舍**  

- **快速启动 vs 峰值性能**：字节码保证快速启动，TurboFan 优化热代码。  
- **内存 vs CPU**：分代 GC 平衡回收效率和停顿时间。  

**可能追问的问题**

1. **Q: 如何查看 V8 的字节码？**  
   
   **A**: 使用 Node.js 的 `--print-bytecode` 标志（需调试版本 V8）。  
   
2. **Q: 为什么避免使用 `arguments` 对象？**  
   
   **A**: `arguments` 破坏隐藏类优化，改用剩余参数（`...args`）。  
   
3. **Q: V8 如何处理异步代码？**  
   
   **A**: V8 只执行同步代码和回调，异步调度由宿主环境（如浏览器事件循环）管理。  
   
4. **Q: 如何触发垃圾回收？**  
   
   **A**: 开发者无法直接触发，但可通过 `global.gc()`（Node.js 启用 `--expose-gc`）测试。  



## 谈谈Javascript的垃圾回收机制

垃圾回收（Garbage Collection，简称 GC）是编程语言自动管理内存的核心机制，主要任务是**识别并释放不再被使用的内存**，避免内存泄漏和程序崩溃。在 JavaScript 中，开发者无需手动释放内存（如 C 语言的`free()`），而是由引擎（如 V8）自动完成这一过程。以下是对 GC 的详细解析：

**一、核心概念：如何判断「垃圾」？**

GC 的第一步是确定哪些内存是「垃圾」（不再需要的）。主流的判断方法是**可达性分析（Reachability）**：

- **根对象（Roots）**：全局变量、当前调用栈中的变量、寄存器中的值等，这些是程序的「入口点」。
- **可达对象**：从根对象出发，通过引用链（如对象的属性、数组的元素）能访问到的对象，视为「存活」。
- **不可达对象**：无法从根对象到达的对象，视为「垃圾」，需要被回收。

**二、常见 GC 算法**

不同场景下，GC 会选择不同的算法，核心目标是平衡「回收效率」和「内存利用率」。

**1. 引用计数（Reference Counting）**

**原理**：每个对象记录被引用的次数，当引用数为 0 时回收。
**优点**：实时性高（引用数为 0 时立即回收），无需暂停程序。
**缺点**：

- 无法处理循环引用（如`a.b = b; b.a = a`，两者引用数始终 > 0）。
- 维护引用计数的开销大（每次赋值都要更新计数）。
  **现状**：现代浏览器已淘汰，仅部分早期引擎（如 IE 的 COM 对象）使用。
...
- **清除阶段**：遍历内存，回收未标记的对象（垃圾）。
  **优点**：解决循环引用问题，实现简单。
  **缺点**：
- 内存碎片化（回收后产生不连续的空闲内存块）。
- 清除阶段需遍历整个内存，耗时随内存增大而增加。

**3. 标记 - 整理（Mark-Compact）**

**原理**：在标记 - 清除的基础上，将存活对象向内存一端移动，整理出连续的空闲空间。
**优点**：解决内存碎片化问题。
**缺点**：整理阶段需移动对象，开销更大。

**4. 复制算法（Copying）**

**原理**：将内存分为大小相等的两块（From 空间、To 空间），每次只使用一块：

- 标记阶段：标记存活对象。
- 复制阶段：将存活对象复制到 To 空间，然后交换 From 和 To 空间，清空原 From 空间。
  **优点**：内存连续（无碎片），回收效率高（只需处理存活对象）。
  **缺点**：内存利用率低（仅使用 50%）。

**三、V8 引擎的 GC 策略：分代回收**

V8 将内存分为**新生代（Young Generation）\**和\**老生代（Old Generation）**，根据对象的生命周期采用不同的 GC 算法，兼顾效率和内存利用率。

**1. 新生代（短生命周期对象）**

- **存储对象**：短寿对象（如函数内的临时变量、局部对象），占内存较小（通常 1-8MB）。
- **GC 算法**：基于复制算法的**Scavenge**。
- **具体流程**：
  - 新生代分为**From 空间**（当前使用）和**To 空间**（空闲）。
  - 当 From 空间满时，触发 Minor GC（新生代 GC）：
    - 标记存活对象（从根对象出发，或通过「写屏障」跟踪引用）。
    - 将存活对象复制到 To 空间（按顺序排列，避免碎片）。
    - 交换 From 和 To 空间，清空原 From 空间。
  - **晋升机制**：若对象存活过多次 Minor GC（如 2 次），会被移动到老生代（称为「对象晋升」），避免频繁复制。

**2. 老生代（长生命周期对象）**

- **存储对象**：长寿对象（如全局变量、闭包、DOM 节点），占内存较大（可达 GB 级别）。
- **GC 算法**：标记 - 清除（Mark-Sweep）+ 标记 - 整理（Mark-Compact）。
- **具体流程**：
  - **标记阶段**：通过「增量标记」（Incremental Marking）逐步标记存活对象（分多次完成，减少停顿）。
  - **清除 / 整理阶段**：
    - 大部分情况使用标记 - 清除（快速回收）。
    - 内存碎片过多时，触发标记 - 整理（移动存活对象，合并空闲空间）。

**3. V8 的优化技术**

为减少 GC 对主线程的阻塞（Stop The World），V8 采用以下优化：

- **增量标记**：将标记过程拆分为多个小步骤，穿插在程序执行中，减少单次停顿时间。
- **并发回收**：利用后台线程协助标记和清理（如 Chrome 的 Worker 线程），主线程无需等待。
- **惰性清理**：标记完成后，不立即清理所有垃圾，而是按需释放（访问到空闲内存时再清理）。

**四、开发者需要注意的「内存泄漏」场景**

GC 虽自动，但错误的代码仍可能导致内存泄漏（垃圾未被回收）。常见场景：

**1. 意外的全局变量**

未声明的变量会被绑定到全局对象（如`window`），导致长期存活：

```javascript
function leak() {
  a = 1; // 等价于 window.a = 1，全局变量
}
```

**2. 未移除的事件监听器**

为 DOM 元素添加事件监听器后，若未移除且元素被删除，监听器可能仍引用元素：

```javascript
const button = document.getElementById('btn');
button.addEventListener('click', handleClick);
// 即使 button 被移除，handleClick 仍可能持有其引用（取决于引擎实现）
```

**3. 闭包滥用**

闭包会捕获外部作用域的变量，若闭包长期存在（如全局函数），被捕获的变量无法回收：

```javascript
function createClosure() {
  const largeData = new Array(1e6).fill(0); // 大数组
...
`setInterval` 回调若引用了不再需要的对象，会导致对象无法回收：

```javascript
const obj = { data: '重要数据' };
setInterval(() => {
  console.log(obj.data); // obj 被定时器引用，无法回收
}, 1000);
```

**五、如何排查内存泄漏？**

通过浏览器工具（如 Chrome DevTools）可定位内存问题：

1. **Memory 面板**：记录内存快照，对比前后差异，找出异常增长的对象。
2. **Performance 面板**：观察 GC 时间和频率，判断是否频繁触发 GC。
3. **Heap Snapshot**：分析对象的引用链，定位未被释放的根引用。

**可能的追问及回答**

**追问 1：为什么分代回收能提升 GC 效率？**
答：根据「弱代假说」（大部分对象生命周期很短），将对象按年龄分代处理：新生代用复制算法快速回收短寿对象（牺牲空间换时间），老生代用标记 - 清除 / 整理处理长寿对象（减少复制开销）。这种策略平衡了回收效率和内存利用率。

**追问 2：GC 停顿是怎么产生的？V8 如何减少停顿？**
答：早期 GC 在标记或整理阶段需要暂停程序执行（Stop The World），避免回收过程中对象引用变化。V8 通过增量标记（拆分标记步骤）、并发回收（后台线程协助）、惰性清理（延迟清理）等技术，将停顿时间从「百毫秒级」缩短到「几毫秒级」。

**追问 3：手动触发 GC（如`window.gc()`）可行吗？**
答：不可行。现代浏览器默认禁用手动 GC（如 Chrome 需通过`--expose-gc`参数启用），且手动触发会干扰引擎的自动调优策略。开发者应通过优化代码避免内存泄漏，而非依赖手动回收。

## 说说 Web Worker

Web Worker 是浏览器提供的 JavaScript 多线程解决方案，主要解决主线程被复杂计算阻塞的问题。简单来说，它能让我们在后台线程运行脚本，避免页面卡顿。以下是我对它的核心理解：

**一、核心作用与原理**

- **为什么需要它**：JS 是单线程语言（主线程同时负责渲染和脚本执行），复杂计算（如大数据排序、图像滤镜）会阻塞主线程，导致页面卡顿。Web Worker 提供一个独立的后台线程，分担计算压力。
- **工作原理**：创建 Worker 时，浏览器会启动一个独立的线程，运行指定的 JS 文件。主线程与 Worker 通过**消息传递**（`postMessage` 和 `onmessage`）通信，数据通过拷贝传递（部分对象可转移所有权）。

**二、关键特性**

1. **独立上下文**
   Worker 有自己的全局作用域（`self`），但无法直接访问主线程的 DOM、`window`、`document` 等对象（避免多线程操作 DOM 导致的竞态条件）。
2. **支持的 API**
   可以使用 `fetch`、`XMLHttpRequest`、`WebSockets`、定时器（`setTimeout`）等，但不能操作 DOM。
3. **通信机制**
   - 主线程与 Worker 双向通信，通过 `postMessage(data)` 发送消息，`onmessage` 监听消息。
   - 数据默认**深拷贝**（结构化克隆），大对象传递可能影响性能；可通过 `Transferable` 对象转移二进制数据所有权（如 `ArrayBuffer`），避免拷贝。

**三、典型使用场景**

- **复杂计算**：如金融数据计算、图像像素处理、AI 模型推理（需配合 `OffscreenCanvas`）。
- **数据预处理**：大数据量的排序、过滤（如 CSV 文件解析）。
- **轮询请求**：后台定时请求接口（如监控数据更新），不阻塞页面交互。

**四、注意事项**

1. **同源限制**：Worker 脚本必须与主线程页面同源（相同协议、域名、端口）。
2. **内存限制**：每个 Worker 有独立内存空间，浏览器对总内存有限制（如 Chrome 通常限制为几个 GB），需避免创建过多 Worker。
3. **生命周期**：Worker 需手动终止（`worker.terminate()`）或页面关闭时自动销毁，否则可能占用资源。
4. **错误处理**：Worker 内的错误不会冒泡到主线程，需通过 `onerror` 监听（或在 Worker 内用 `try/catch`）。

**可能的追问及回答**

**追问 1：Web Worker 和主线程如何通信？数据是共享还是拷贝？**
答：通过 `postMessage` 和 `onmessage` 通信。默认情况下，数据会被深拷贝（结构化克隆算法），主线程和 Worker 持有独立副本。若传递大对象（如 `ArrayBuffer`），可通过 `Transferable` 转移所有权（原线程失去访问权），避免拷贝开销。

**追问 2：Web Worker 能访问 DOM 吗？为什么？**
答：不能。因为 JS 是单线程设计，多线程同时操作 DOM 会导致竞态条件（如两个线程同时修改同一个元素），浏览器无法保证一致性。因此，Worker 被严格限制访问 DOM。

**追问 3：Web Worker 和 Service Worker 有什么区别？**
答：核心区别在**用途和生命周期**：

- Web Worker：用于后台计算，与页面绑定（页面关闭则销毁），只能被创建它的页面通信。
- Service Worker：用于离线缓存、拦截网络请求，是浏览器级别的代理服务（可在页面关闭后继续运行），支持全局事件（如 `fetch`、`push`）。

**追问 4：如何优化 Web Worker 的性能？**
答：

- 避免频繁创建 / 销毁 Worker（复用 Worker 实例）。
- 传递大对象时使用 `Transferable` 转移所有权（如 `ArrayBuffer`）。
- 将任务分块（如分批处理数据），避免单个任务阻塞 Worker 线程。

## WebWorker、SharedWorker 和 ServiceWorker 有哪些区别？

的共同点，比如都是浏览器提供的多线程机制，都不能直接操作 DOM 等。

这三个 Worker 都是浏览器提供的「后台线程」方案，但定位和用途差异很大。面试时可以从**核心用途、生命周期、通信方式、作用域**四个维度对比，更清晰易懂：

**一、核心用途对比**

- **Web Worker**：解决单线程阻塞问题，专注**复杂计算**（如大数据处理、数学运算）。
  举例：页面需要对 10 万条数据排序，用 Web Worker 在后台计算，避免卡顿。
- **Shared Worker**：解决**同源页面间通信**，多个页面共享同一个 Worker 实例。
  举例：同一网站的两个标签页需要同步购物车数据，用 SharedWorker 作为「共享仓库」。
- **Service Worker**：作为浏览器的「网络代理」，专注**离线缓存、请求拦截、推送通知**（PWA 核心技术）。
  举例：实现离线访问、缓存静态资源、拦截 HTTP 请求返回自定义内容。

**二、生命周期对比**

| **类型**       | **生命周期触发条件**                     | **典型结束时机**                     |
| -------------- | ---------------------------------------- | ------------------------------------ |
| Web Worker     | 页面通过`new Worker()`创建               | 页面关闭或调用`worker.terminate()`   |
| Shared Worker  | 任意同源页面通过`new SharedWorker()`连接 | 所有关联页面关闭                     |
| Service Worker | 页面注册后，经「安装→激活」阶段启动      | 手动注销（`unregister`）或浏览器清理 |

**三、通信方式对比**

- **Web Worker**：**一对一通信**（主线程 ↔ 专属 Worker）。
  用`postMessage`发送消息，`onmessage`监听，数据默认深拷贝（大对象可转移所有权）。
- **Shared Worker**：**多对一通信**（多个页面 ↔ 同一个 Worker）。
  页面通过`port`连接 Worker（`worker.port.start()`），Worker 需监听`connect`事件管理连接，通信时需指定`port`。
- **Service Worker**：**广播式通信**（页面 ↔ Service Worker ↔ 其他页面）。
  页面通过`postMessage`发送消息，Service Worker 用`self.clients`管理所有关联页面，支持广播（如`clients.matchAll().then(clients => clients.postMessage(...))`）。

**四、关键限制对比**

| **特性**     | Web Worker         | Shared Worker       | Service Worker                            |
| ------------ | ------------------ | ------------------- | ----------------------------------------- |
| **DOM 访问** | ❌（无`window`）    | ❌（无`window`）     | ❌（无`window`）                           |
| **作用域**   | 仅创建它的页面     | 所有同源页面        | 注册时指定的作用域（如`/`匹配所有子路径） |
| **网络请求** | ✅（支持`fetch`等） | ✅（支持`fetch`等）  | ✅（可拦截所有请求）                       |
| **持久化**   | ❌（随页面销毁）    | ❌（随所有页面销毁） | ✅（可长期运行）                           |

**五、注意事项**

- **Web Worker**：避免创建过多（浏览器对 Worker 数量有限制），大对象传递用`Transferable`（如`ArrayBuffer`）减少拷贝开销。

- **Shared Worker**：需处理多页面并发连接（如加锁避免数据冲突），且兼容性不如 Web Worker（部分浏览器支持不佳）。

- **Service Worker**：需 HTTPS（开发环境可本地`localhost`），注册后需处理「安装失败」「激活更新」等事件（如缓存版本控制）。

  

**可能的追问及回答**

**追问 1：Shared Worker 如何实现多页面通信？**
答：页面通过`new SharedWorker('script.js')`连接同一个 Worker，Worker 内监听`connect`事件获取`port`对象（每个页面一个`port`）。页面和 Worker 通过`port.postMessage()`通信，Worker 需遍历所有`port`广播消息。

**追问 2：Service Worker 离线缓存的原理是什么？**
答：Service Worker 注册后，会拦截页面的所有网络请求（通过`fetch`事件）。开发者可自定义策略：若请求在缓存中存在，直接返回缓存；否则请求网络并更新缓存。

**追问 3：这三个 Worker 都不能访问 DOM，为什么？**
答：JS 是单线程设计，多线程同时操作 DOM 会导致竞态条件（如两个线程同时修改同一个元素），浏览器无法保证数据一致性。因此，所有 Worker 都被限制访问 DOM。
...
JSON（JavaScript Object Notation）是一种轻量级的数据交换格式，广泛用于前后端通信。以下是我对它的核心理解：

**一、核心特性**

1. **语法简洁**
   - 基于 JavaScript 对象字面量，但**严格遵循键值对格式**（键必须用双引号）。
   - 支持的数据类型：`object`、`array`、`string`、`number`、`boolean`、`null`。
2. **跨语言兼容**
   - 几乎所有编程语言都有解析 JSON 的库，成为 API 数据传输的事实标准。
3. **与 JS 对象的区别**
   - JSON 是**字符串**，需解析后才能成为 JS 对象。
   - JS 对象更灵活（如支持函数、正则等非标准类型），而 JSON 必须严格符合规范。

**二、常见应用场景**

1. **API 数据传输**

   - 前后端通过 JSON 交换数据（如 REST API 返回 JSON 格式响应）。

2. **配置文件**

   - 如`package.json`、`tsconfig.json`等，比 XML 更简洁。

3. **本地存储**

   - `localStorage`只能存储字符串，常用`JSON.stringify/parse`处理复杂数据：
...
1. **JSON.stringify(obj[, replacer[, space]])**
   ...
   - 注意点：
     ...
     - 函数、`undefined`、`Symbol`会被忽略（数组中会转为`null`）。

     - 循环引用会报错（需手动处理）。

     - 可通过`replacer`过滤属性，`space`控制缩进：

       ```javascript
       JSON.stringify({ a: 1, b: 2 }, ['a'], 2); // 只保留'a'属性，缩进2空格
       ```
...
2. **JSON.parse(str[, reviver])**
   ...
   - **安全风险**：
     避免解析不可信的 JSON（如用户输入），可能导致原型污染攻击。

     ```javascript
     JSON.parse('{"__proto__":{"x":1}}'); // 可能污染全局原型
     ```
...
1. **严格格式要求**
   ...
   - 键必须用双引号（如`{"name":"John"}`），单引号或无引号会解析失败。
   - 不允许末尾逗号（如`{a:1,}`）。

2. **特殊值处理**

   - `NaN`、`Infinity`、`Date`对象等需手动转换：

     ```javascript
     const date = new Date();
     JSON.stringify(date); // 转为ISO字符串 "2023-10-01T00:00:00.000Z"
     ```

3. **性能考量**

   - 大对象序列化 / 解析可能耗时，可考虑使用`Map`、`Set`等原生数据结构替代。

**可能的追问及回答**

**追问 1：JSON 和 JS 对象的区别是什么？**
答：JSON 是一种文本格式（字符串），有严格的语法规范；JS 对象是运行时数据结构。例如：

- JS 对象：`{ name: 'John', age: undefined }`
- JSON：`{"name":"John"}`（`undefined`会被忽略）

**追问 2：如何处理 JSON 中的循环引用？**
答：可使用`JSON.stringify`的`replacer`参数过滤循环引用：

```javascript
const seen = new Set();
JSON.stringify(obj, (key, value) => {
  if (typeof value === 'object' && value !== null) {
    if (seen.has(value)) return; // 跳过已处理的对象
    seen.add(value);
  }
  return value;
});
...
**追问 3：JSON 和 XML 的优缺点对比？**
答：

- **JSON**：轻量、解析快、更适合 JS，但不支持注释和命名空间。
- **XML**：更结构化、支持 DTD 验证，但冗余标签导致体积大。
...
**追问 1：Base64 与加密算法的区别？**
答：Base64 是编码（可逆转换），不是加密。它仅改变数据表示形式，无密钥保护，无法防止数据泄露。

**追问 2：为什么 Base64 会使数据膨胀？**
答：原始数据按每 3 字节拆分为 4 个字符，因此体积变为原数据的 **4/3 ≈ 133%**。

(计算机存储数据的基本单位是 **8 位字节**，原本二进制数据可以将8位存储满，但是转换成Base64字符（A-Z, a-z, 0-9, +/），一般都是6位，所以一个字节只存了6位有效数据，因此体积增加了1/3)

**追问 3：URL Safe Base64 是什么？**
答：标准 Base64 中的 `+`、`/`、`=` 在 URL 中需转义（如 `+` → `%2B`），导致冗余。URL Safe 版本用 `-` 替换 `+`，`_` 替换 `/`，并移除 `=`，避免转义。

**追问 4：如何优化 Base64 图片的性能？**
答：

1. 仅对小图片使用（如 1KB 以下），避免大文件膨胀过度。
2. 结合 HTTP/2 多路复用，减少请求数。
3. 使用工具（如 `image-webpack-loader`）自动转换并压缩。
...
**追问 1：为什么 `NaN === NaN` 返回 `false`？**
答：这是 JavaScript 早期设计的历史遗留问题。`NaN` 表示非数字，且规范规定 **任何值（包括自身）与 `NaN` 比较均返回 `false`**。需用 `isNaN()` 或 `Object.is()` 判断。

**追问 2：`Object.is()` 与 `===` 的性能差异？**
答：`Object.is()` 略慢（因多了符号和 `NaN` 判断），但现代引擎优化后差异可忽略不计。优先考虑语义正确性而非性能。

**追问 3：`Object.is()` 能替代 `===` 吗？**
答：大多数场景可用 `Object.is()` 替代，但需注意其对 `+0` 和 `-0` 的区分可能导致意外结果（如 `Object.is(0, -0)` → `false`）。
...
1. **命名空间隔离**
   - 使用唯一前缀（如`app_`/`myLib_`）避免命名冲突
   - 对于复杂逻辑，封装为 IIFE（立即执行函数表达式）或类，通过单一出口暴露接口。
2. **限制挂载内容的生命周期**
   - 非必要不长期保留全局引用，如临时工具函数可在使用后删除：
3. **区分只读与可写属性**
   - 对常量或配置项使用`Object.freeze()`冻结，避免意外修改
4. **兼容性考量**
   - 避免覆盖浏览器原生方法（如`window.alert`），低版本浏览器可能无法恢复默认行为。

**可能的追问及回答**

**追问 1：在 Node.js 中挂载到 global 对象是否有类似风险？**
答：

- 原理类似，但 Node.js 的`global`作用域仅在当前模块有效（CommonJS 规范），风险相对较低。但在 ES Module 中，`globalThis`为全局作用域，需注意同样的命名冲突问题。

**追问 2：如何安全地向 window 挂载内容？**
答：

- 优先使用模块化方案（如 ES6 的`export/import`），仅在必要时（如第三方脚本注入）挂载。
- 挂载前检查属性是否已存在，或通过Symbol创建唯一键：

**追问 3：SPA 项目中挂载全局状态（如 store）是否推荐？**
答：

- 不推荐直接挂载`window.store`，应通过状态管理库（如 Redux/Vuex）或上下文（Context）机制管理。全局挂载会破坏组件封装性，且难以追踪状态变化。

## 原生 js 如何进行监听路由的变化？

原生 JS 监听路由变化主要有两种场景：**Hash 路由**和**History API 路由**。以下是核心方法和注意事项：

**一、监听 Hash 变化（# 后的内容）**

**1. 核心方法**

- **`hashchange` 事件**：当 URL 的 hash 值（如 `http://a.com#/page1` 中的 `/page1`）发生变化时触发。  
  ```javascript
  window.addEventListener('hashchange', (event) => {
    console.log('新的 hash:', location.hash);
    console.log('旧的 hash:', event.oldURL.split('#')[1]);
  });
  ```

- **初始化检测**：页面加载时需手动触发一次回调，处理初始 hash 值。  
  ```javascript
  // 页面加载时执行一次
...
**二、监听 History API 变化（pushState/replaceState）**

**1. 核心问题**

- `pushState` 和 `replaceState` **不会触发任何事件**，需手动劫持这两个方法。

**2. 实现方案**

- **重写原生方法**：在调用原生方法前触发自定义事件。  
  ```javascript
  // 保存原始方法
  ...
  window.addEventListener('pushstate', () => {
    console.log('pushState 被调用，新路径:', location.pathname);
  });
  ```

- **监听 `popstate` 事件**：  
  - 仅在用户点击浏览器前进/后退按钮时触发，对 `pushState/replaceState` 无效。  
  ```javascript
  window.addEventListener('popstate', (event) => {
    console.log('历史记录变化，状态:', event.state);
  });
  ```
...
1. **兼容性**  
   - `hashchange`：IE8+、Chrome、Firefox 均支持。  
   - History API：IE10+，需注意低版本浏览器兼容。

2. **性能优化**  
   - 避免在事件回调中执行大量计算，可使用 `requestAnimationFrame` 优化。  
   ```javascript
...
3. **路径解析**  
   - 使用 `URL` 对象解析路径，避免手动处理字符串。  
   ```javascript
   const url = new URL(window.location);
   console.log('路径:', url.pathname);
   console.log('查询参数:', url.searchParams.get('id'));
   ```

**可能的追问及回答**

**追问1：如何实现一个简易的前端路由？**  
答：结合 `hashchange`/`popstate` 监听和路由配置表，示例逻辑：  
```javascript
const routes = {
  '/': () => { /* 首页逻辑 */ },
  '/about': () => { /* 关于页逻辑 */ }
...
**追问2：`pushState` 和 `replaceState` 的区别？**  
答：  
- `pushState`：添加新的历史记录，点击后退可返回上一页。  
- `replaceState`：替换当前历史记录，点击后退会跳过当前页。

**追问3：监听路由变化会影响性能吗？**  
答：合理实现不会显著影响性能，但需注意：  

- 避免在事件回调中频繁操作 DOM。  
- 对复杂计算使用防抖（debounce）或节流（throttle）。
...
5. **异步任务协调** 结合 Promise 实现等待响应模式。
   ...
   **可能的追问及回答：**
   ...
   1. **追问：MessageChannel 和 postMessage 有什么区别？**
      回答：`postMessage` 是单向通信（如窗口间或主 - Worker 间），而 MessageChannel 是双向通道，且可通过两个端口灵活控制消息流向；此外，MessageChannel 的事件监听更聚焦，不易受全局消息干扰。
   2. **追问：为什么需要双向通信？单向不能解决吗？**
      回答：当需要实现请求 - 响应模式（如 A 发送消息给 B，B 处理后返回结果给 A）时，双向通道更直接，无需通过第三方变量或全局事件中转，代码逻辑更清晰。
   3. **追问：在 Web Worker 中使用 MessageChannel 有什么优势？**
      回答：默认的 `postMessage` 通过全局 `onmessage` 监听，多个 Worker 或通信场景可能导致事件处理混乱。MessageChannel 的端口绑定更精准，可避免监听逻辑冲突，尤其适合复杂的多 Worker 协作场景。
...
- **延时并非真正为 0**：浏览器最小延时通常有默认限制（如 HTML 规范中最短 4ms），实际延迟可能略大于 0。
- **不要滥用**：过度使用会导致任务队列堆积，反而影响性能，需结合具体场景权衡。

**可能的追问及回答：**

1. **追问：setTimeout (0) 和 Promise.then () 有什么区别？**
   回答：前者属于宏任务，回调在宏任务队列中执行；后者属于微任务，会在当前宏任务末尾、下一个宏任务前执行。微任务优先级更高，执行时机更接近同步代码。
2. **追问：为什么设置 0 延时能避免阻塞？**
   回答：JS 是单线程，同步代码会阻塞主线程。`setTimeout(0)` 将回调放入宏任务队列，浏览器在完成当前同步任务（如渲染、事件处理）后，才会执行队列中的回调，从而 “让出” 主线程控制权。
...
**深度优先遍历（DFS）**  
- **策略**：优先遍历子节点的子树，直到最深层再回溯。  
- **实现**：递归或栈（Stack）。  
- **场景**：适合需要遍历到最深层节点的场景（如查找特定类型的深层元素）。  

**广度优先遍历（BFS）**  
- **策略**：逐层遍历同级节点，再深入下一层。  
- **实现**：队列（Queue）。  
- **场景**：适合寻找最短路径或层级相关的操作（如查找最近的祖先节点）。  
...
1. **性能差异**  
   - DFS 递归实现可能导致栈溢出（如 DOM 树过深），迭代实现更安全。  
   - BFS 使用队列，空间复杂度更高（尤其宽而浅的树）。  

2. **遍历顺序**  
   - DFS：根 → 左子树 → 右子树（前序遍历，最常见）。  
   - BFS：按层级从根到叶，同一层级按子节点顺序。  

3. **DOM 动态变化**  
   - 遍历时修改 DOM 结构（如删除节点）可能导致意外结果，需谨慎处理。  

**可能的追问及回答**

**追问1：如何在遍历中过滤特定节点？**  
答：在处理节点时添加条件判断，如 `if (node.tagName === 'DIV') { /* 处理逻辑 */ }`。  

**追问2：BFS 和 DFS 哪个更适合查找最近的匹配节点？**  
答：BFS。因其逐层遍历，首次匹配到的节点层级最低，路径最短。  

**追问3：如何遍历 Shadow DOM？**  
答：需显式访问 `element.shadowRoot`，并递归遍历其内部 DOM 树。  

**追问4：是否存在原生 API 实现 BFS/DFS？**  
答：有 `TreeWalker` 和 `NodeIterator`，但手动实现更灵活，适合面试场景。  
...
**追问 1：如何让结果变成 [1, 2, 3]？**
答：显式指定 `parseInt` 的进制为 10，避免索引干扰：

```javascript
["1", "2", "3"].map((v) => parseInt(v, 10)); // [1, 2, 3]
```
